...
You can use the xtrasonnet IntelliJ Plugin in any IntelliJ project, but you'll set one up just for this tutorial.
...
Now that the basic project is created and the plugin is installed, you'll create the first xtrasonnet transformation and scenario for experimentation.
...
To fix that, you need to update your xtrasonnet file, which is the currently empty middle pane.

=== The minimal transformation

In the empty middle pane, put two curly brackets `{}`.
This is a minimal xtrasonnet transformation, though it does not use the input data at all.
...
The main way to transform arrays in xtrasonnet is called an array comprehension.
...
xtrasonnet is much stricter about issues like this than Javascript.
...
So far the xtrasonnet transformation is a single expression with a number of parts, but accomplishing complex transformations requires the ability to break down code into smaller pieces.
...
xtrasonnet makes that easy by supporting object combination.
Update the transformation to
...
Combining objects like this is useful but limited, with fields replacing other fielxtr.
xtrasonnet supports much more powerful operations when combining objects.
Update the transformation to
...
The JSonnet language xtrasonnet uses includes many more powerful capabilities.
